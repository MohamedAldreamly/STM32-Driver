...
{
	//HPRE: AHB prescaler
		CLR_4BIT( MRCC->CFGR , RCC_HPRE_POSITION );
		SET_4BIT_VALUE( MRCC->CFGR , RCC_HPRE_POSITION , RCC_AHB_PRESCALER);
		
		//PPRE1: APB1 prescaler
		CLR_3BIT( MRCC->CFGR , RCC_PPRES1_POSITION );
		SET_3BIT_VALUE( MRCC->CFGR , RCC_PPRES1_POSITION , RCC_APB1_PRESCALER );

		//PPRE2: APB2 prescaler
		CLR_3BIT( MRCC->CFGR , RCC_PPRES2_POSITION );
		SET_3BIT_VALUE( MRCC->CFGR , RCC_PPRES2_POSITION , RCC_APB2_PRESCALER );

	#if  RCC_CLOCK_TYPE == RCC_HSI
		// Enable HSI
		SET_BIT( MRCC->CR , RCC_HSION );
		// Wait Until HSI Is Ready ( use timeout ) 
		while ( GET_BIT( MRCC->CR , RCC_HSIRDY ) == 0 );

		// Select HSI As System Clock
		CLR_2BIT( MRCC->CFGR , RCC_SW_POSITION );
		SET_2BIT_VALUE( MRCC->CFGR , RCC_SW_POSITION , RCC_SW_HSI );
	#elif RCC_CLOCK_TYPE == RCC_HSE
		// Enable HSE
		SET_BIT( MRCC->CR , RCC_HSEON );
		// Wait Until HSE Is Ready 
		while ( GET_BIT( MRCC->CR , RCC_HSERDY ) == 0 );
		// Select HSE As System Clock
		CLR_2BIT( MRCC->CFGR , RCC_SW_POSITION );
		SET_2BIT_VALUE( MRCC->CFGR , RCC_SW_POSITION , RCC_SW_HSE );
	#elif RCC_CLOCK_TYPE == RCC_PLL

		/* Configure PLL source  */

		#if  PLL_SOURCE == RCC_PLL_HSI_DIVIDED_BY_2
		...
		#elif PLL_SOURCE == RCC_PLL_HSE_DIVIDED_BY_2
			SET_BIT( MRCC->CFGR , RCC_PLLSRC_POSITION );
			SET_BIT( MRCC->CFGR ,RCC_PLLXTPRE_POSITION);
			#endif 
		...
		// Enable PLL
		SET_BIT( MRCC->CR , RCC_PLLON );
		// Wait Until PLL Is Ready
		while ( GET_BIT( MRCC->CR , RCC_PLLRDY ) == 0 );

		 /* Select PLL as SYSCLK */
		CLR_2BIT( MRCC->CFGR , RCC_SW_POSITION );
		SET_2BIT_VALUE( MRCC->CFGR , RCC_SW_POSITION , RCC_SW_PLL );	
